	...
	<ul>
		...
		<view class="back" v-for="(item, index) in Rendering" :key="index" @tap="Jump(item)">
			<view class="backImage">
				<image :src="item.imageUrl" mode="aspectFill" style="width: 115rpx;height: 113rpx;border-radius: 50%;"></image>
			</view>
			<view class="backText">
	...
	async test(){
		try{
			...
			if (res.data.status == 100){
				...
				// }
				const filteredData = this.IntelligentData.filter(item => item.category === this.$store.state.Intelligent_index);
