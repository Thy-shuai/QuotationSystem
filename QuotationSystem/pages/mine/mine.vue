...
import { plus } from '../../uni_modules/uview-ui/libs/function/digit.js';
	export default {
		data() {
			return {
				form:[
		...
		methods: {
			...
			async getUserInformation(){
				try{
					...
					this.nickName = res.data.data.username.replace(/(\d{3})\d{4}(\d{4})/, '$1 **** $2');
