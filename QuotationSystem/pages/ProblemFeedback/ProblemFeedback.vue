		...
		loadImageToCanvas(url) {
		  ...
		  uni.downloadFile({
			...
			success: (res) => {
				...
				// 确保文字在画布内绘制，调整 y 坐标
				ctx.fillText('刘 帅', 25, 50); // 在图片上绘制文字
				
				ctx.setFontSize(12);
				ctx.setFillStyle('#a7a7a7');
				ctx.fillText('专业医学顾问', 80, 50);
				
				
				ctx.setFontSize(12);
				ctx.setFillStyle('#000000');
				ctx.fillText('灵犀报价科技有限公司', 25, 95);
				// 绘制圆形
				// 圆形半径
				let radius = 9;
				let introduce = ['电话：18352868816','地址：深圳市南山区金色商业街001号', '官网：www.canva.com'];
				let icon = ['/static/telephone2.png', '/static/address.png', '/static/email.png']
				
				for (let i = 0; i < introduce.length ;i++){
					ctx.beginPath();
					ctx.arc(25+radius, 115 + i * 20, radius, 0, Math.PI * 2); // x=200, y=200, 半径=100
					ctx.fillStyle = '#0a7d94'; // 圆形背景颜色
					ctx.fill();
					ctx.closePath();
					
					ctx.drawImage(icon[i], 25 + radius / 2, 115 + i *20 -radius / 2, radius, radius); 
					
					ctx.setFontSize(10);
					ctx.setFillStyle('#2C405A');
					ctx.fillText(introduce[i], 25+3*radius-4, 115 + i * 20+4);
				}
				// 完成绘制并刷新画布
				ctx.draw(true, () => {
					// 可以将画布内容转为图片并输出
